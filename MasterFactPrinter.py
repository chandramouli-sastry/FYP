...
def gen_simple(writer):
    ...
    for field in fields:
        ...
        if os.path.exists(path):
            try:
                x=SimpleFactPrinter(json.load(open(path)),writer=writer)
                x.process()
...
def gen_semantic(writer):
    ...
    for field in fields:
        ...
        if os.path.exists(path):
            try:
                x=BinarizedSemanticFactPrinter(json.load(open(path)),writer=writer)
                x.process()
            except Exception as e:
                print("Couldnt do",e)
        count += 1
...
def gen_ratio(writer):
    ...
    for score,field_1,field_2 in field_pairs:
        ...
        if os.path.exists(path):
            try:
                x=RatioFactPrinter(json.load(open(path)),writer=writer)
                x.process()
            except Exception as e:
                print("Couldnt do",e)
        count += 1
...
writer = Writer(type="list")
writer_simple = Writer(type="list")
...
def process(file_name):
    w_simple = json.load(open(file_name))
    w_simple = sorted(w_simple,reverse=True)
    for i in w_simple[:100]:
        print(i[1])
        print("\t",i[2])
        print("\t",i[3])
