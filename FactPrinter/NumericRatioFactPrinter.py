...
from FactPrinter.QuartileCalculation import *
from FactPrinter.PercentageRangeDict import *
from FactPrinter.testdata import *
...
def global_perc_trend_statistics(perc_range_dict, field1, field2, trend_ratio, partition_field):
    for ranges in perc_range_dict:
        ...
        if list_of_fields_gps:
            ...
            if bytes('', "utf-8") in state_list:
                continue
            if trend_ratio != 9999:
                print("Approximately " + str(avg_percentage) + "% villages in the states of " + str(state_list) + " have the " \
                                                                                                                  "ratio between " + field1 + " and " + field2 + " as " + str(
                    trend_ratio))
            else:
                print("Approximately " + str(avg_percentage) + "% villages in the states of " + str(state_list) + " have " \
    ...
    for trend_no in range(0, len(data_dict)):
        ...
        print((precedent_generator(trend_perc_dict[trend_no], q1, q3) + str(
            round(trend_perc_dict[trend_no], 4)) + "% villages have " + field1
               + " = " + str(data_dict[trend_no][data][0][1]) + " and " + field2 + " = " + str(
            data_dict[trend_no][data][1][1]) + "."))
...
def compare_factors_based_trend_generate(data_dict, field1, field2, total_no_trends):
    ...
    print("Out of " + str(
        total_no_trends) + " trends, " + field1 + " and " + field2 + " were equal " + equality_factor + " number of times")
...
def run_test():
    with open('Resources/Facts_data-MaleFemale.json', "rb") as data_file:
        data_dict = json.load(data_file)
        # pprint(data_dict)

    sample_data_content = data_dict[0]
    field1, field2 = str(sample_data_content[data][0][0].encode('utf-8')), str(
        sample_data_content[data][1][0].encode('utf-8'))
    partition = sample_data_content[partition_field].encode('utf-8')
    total_no_trends = len(data_dict)

    print("Total number of trends generated based on Ratio between " + field1 + " and " + field2 + " are : " + str(
        total_no_trends))
    separator()
